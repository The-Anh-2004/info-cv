...
def adjustResultCoordinates(polys, ratio_w, ratio_h, ratio_net=2):
    ...
    for poly in polys:
        if poly is not None:
            # Kiểm tra và điều chỉnh kích thước của mảng con
            if poly.shape[1] == 2:
                poly = np.multiply(poly, (ratio_w * ratio_net, ratio_h * ratio_net))
...
def adjustResultCoordinatesForBoxes(boxes, ratio_w, ratio_h, ratio_net=2):
    adjusted_boxes = []
    for box in boxes:
        if box is not None:
            # Kiểm tra và điều chỉnh kích thước của mảng con
            if box.shape[1] == 2:
                box = np.multiply(box, (ratio_w * ratio_net, ratio_h * ratio_net))
                adjusted_boxes.append(box)
            else:
                adjusted_boxes.append(None)
        else:
            adjusted_boxes.append(None)
